...
char *wifiConnection(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(WiFi.status() == WL_CONNECTED ? '1' : '0');
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
      ...
      case '1':
         ...
         if( settings.ssid[0] && settings.wifiPassword[0] ) {
            if( !settings.quiet && settings.extendedCodes ) {
               tilp.print(F("CONNECTING TO SSID "));
               tilp.print(settings.ssid);
            }
            ...
            if( WiFi.status() != WL_CONNECTED ) {
               ...
               yield();
               if( !settings.quiet && settings.extendedCodes ) {
                  tilp.printf("CONNECTED TO %s IP ADDRESS: %s\r\n",
            ...
            if( !settings.quiet && settings.extendedCodes ) {
               tilp.println(F("Configure SSID and password. Type AT? for help."));
            }
            sendResult(R_ERROR);
         }
         break;
...
char *dialNumber(char *atCmd) {
   ...
   yield();
   if( !settings.quiet && settings.extendedCodes ) {
      tilp.printf("DIALLING %s:%u\r\n", host, portNum);
      tilp.flush();
   }
   delay(2000);   // delay for ZMP to be able to detect CONNECT
   if( !tilp.available() && tcpClient.connect(host, portNum) ) {
      connectTime = millis();
      sendResult(R_CONNECT);
      amClient = true;
      state = ONLINE;
...
#if DEBUG
   tilp.print(F("Getting path /"));
   if( path ) {
      tilp.print(path);
   }
   tilp.printf(" from port %u of host %s...\r\n", portNum, host);
#endif
...
char *doAutoAnswerConfig(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.autoAnswer);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doExtended(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.extendedCodes);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *resetToNvram(char *atCmd) {
   tilp.flush();                     // allow for CR/LF to finish
   ESP.restart();
