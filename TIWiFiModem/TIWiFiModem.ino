//
//   TI WiFi modem: an ESP8266 based TI calculator<->WiFi modem
//   based on:
//
...
void loop(void) {
   ...
   switch( state ) {
      ...
      case ONLINE:
         if( tilp.available() ) {       // data from RS-232 to Wifi
            sendSerialData();
         }

         while( tcpClient.available() && !tilp.available() ) { // data from WiFi to RS-232
