...
char *doAutoExecute(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.autoExecute);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doMdnsName(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.mdnsName);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doWiFiPassword(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.wifiPassword);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doSpeedChange(char *atCmd) {
   ...
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.serialSpeed);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
      case '=':
         ...
         if( newSerialSpeed != settings.serialSpeed ) {
            switch( newSerialSpeed ) {
               ...
               case 115200L:
                  sendResult(R_OK);
                  tilp.flush();               // wait for transmit to finish
...
char *doServerPort(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.listenPort);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doLocation(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.location);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doTerminalType(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.terminal);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
...
char *doStartupWait(char *atCmd) {
   switch( atCmd[0] ) {
      case '?':
         ++atCmd;
         tilp.println(settings.startupWait);
         if( !atCmd[0] ) {
            sendResult(R_OK);
         }
         break;
